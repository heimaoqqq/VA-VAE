    ...
    if os.path.isdir(checkpoint_path):
        ...
        if os.path.exists(safetensors_path):
            ...
            try:
                ...
                print(f"❌ pytorch检查点加载失败: {e}")
                return False
        else:
            print("❌ 未找到model.safetensors或pytorch_model.bin文件")
            return False
            ...
            if os.path.exists(safetensors_path):
                ...
                try:
                    ...
                    try:
                        ...
                        print("✅ 检查点加载成功 (safetensors, 非严格模式)")
                        return True
                    except Exception as e:
                        print(f"❌ 检查点加载失败: {e}")
                        return False
                ...
                try:
                    model.load_state_dict(checkpoint, strict=False)
                    print("✅ 检查点加载成功 (pytorch, 非严格模式)")
                    return True
                except Exception as e:
                    print(f"❌ 检查点加载失败: {e}")
                    return False
            else:
                print("❌ 未找到model.safetensors或pytorch_model.bin")
                return False
        else:
            print("❌ 检查点目录不存在")
            return False
