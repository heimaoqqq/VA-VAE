"""
生成并筛选高质量样本脚本 - 双GPU并行版本
支持双GPU协同处理单个用户，特别适合困难用户（如User_28）
"""
import torch
...
def generate_and_filter_parallel(model, vae, transport, classifier, user_id, 
    ...
    """并行版本：每个GPU负责生成目标样本的一部分"""
    
    # 双GPU并行：每个GPU负责一半目标
    world_size = dist.get_world_size() if dist.is_initialized() else 1
    if world_size > 1:
        gpu_target_samples = target_samples // world_size
        if rank < target_samples % world_size:
            gpu_target_samples += 1
    else:
        gpu_target_samples = target_samples
    
    print(f"[GPU{rank}] 负责生成 {gpu_target_samples} 个样本 (总目标: {target_samples})")
    
    # 生成条件设置
    domain_conditions = [{"cfg": cfg_scale, "steps": 300, "name": "optimized"}]
    samples_per_condition = gpu_target_samples
    ...
    with torch.no_grad():
        ...
        for condition in domain_conditions:
            ...
            while condition_collected < condition_target and len(collected_samples) < gpu_target_samples:
                # 生成一批样本
                remaining_total = gpu_target_samples - len(collected_samples)
                ...
                # 创建随机噪声（添加GPU特定的随机种子偏移）
                torch.manual_seed(torch.initial_seed() + rank * 10000 + stats['total_generated'])
                    ...
                    try:
                        ...
                        for candidate in batch_candidates:
                            ...
                            if diversity_score >= diversity_threshold:
                                ...
                                # 保存图像到磁盘（使用GPU rank避免冲突）
                                img_filename = f"gpu{rank}_sample_{len(collected_samples):04d}_conf{candidate['metrics']['confidence']:.3f}_spec{candidate['metrics']['user_specificity']:.3f}.png"
                                ...
                                if len(collected_samples) >= gpu_target_samples:
                                    break
                        ...
                        # 使用简单的状态更新
                        update_progress(len(collected_samples), gpu_target_samples, stats)
                        
                        # 检查是否完成目标
                        if len(collected_samples) >= gpu_target_samples:
                            # 最终更新显示
                            success_rate = len(collected_samples) / stats['total_generated'] * 100 if stats['total_generated'] > 0 else 0
                            print(f"\r[GPU{rank}]User_{user_id:02d}: {len(collected_samples)}/{gpu_target_samples} | 生成:{stats['total_generated']} | 通过:{success_rate:.1f}%", flush=True)
    ...
    if len(collected_samples) > 0:
        ...
        print(f"[GPU{rank}] ✅ User_{user_id:02d} GPU部分完成:")
        print(f"  📊 收集样本: {len(collected_samples)}/{gpu_target_samples} | 总生成: {stats['total_generated']} | 通过率: {final_success_rate:.1f}%")
...
def merge_gpu_results(user_id, output_dir, world_size):
    """合并多个GPU的结果"""
    from pathlib import Path
    import shutil
    ...
    if dist.get_rank() == 0:
        ...
        for gpu_rank in range(world_size):
            gpu_files = sorted(user_dir.glob(f"gpu{gpu_rank}_sample_*.png"))
            all_files.extend(gpu_files)
        
        print(f"[GPU0] 找到 {len(all_files)} 个文件需要合并")
        
        # 重新编号并重命名
        for idx, src_file in enumerate(all_files, 1):
            # 提取置信度和特异性值
            filename_parts = src_file.name.split('_')
            conf_part = filename_parts[-2]  # confX.XXX
            spec_part = filename_parts[-1]  # specX.XXX.png
            
            # 新文件名（去掉gpu前缀）
            new_filename = f"sample_{idx:04d}_{conf_part}_{spec_part}"
            dst_file = user_dir / new_filename
            
            # 重命名文件
            shutil.move(str(src_file), str(dst_file))
        
        print(f"[GPU0] ✅ 合并完成，共 {len(all_files)} 个高质量样本")
...
def main():
    ...
    # 单用户模式检测
    single_user_mode = (len(user_list) == 1)
    
    if single_user_mode and world_size > 1:
        # 单用户模式：所有GPU协同处理同一用户
        user_id = user_list[0]
        print(f"[GPU{rank}] 并行模式: 与GPU协同处理User_{user_id:02d}")
        
        collected = generate_and_filter_parallel(
        ...
        # 合并多GPU结果
        if world_size > 1:
            merge_gpu_results(user_id, args.output_dir, world_size)
    else:
        # 多用户模式：每个GPU处理不同用户
    ...
    # 打印当前GPU完成情况
    if single_user_mode:
        print(f"GPU {rank} 完成: 收集了 {total_collected} 个样本")
    else:
